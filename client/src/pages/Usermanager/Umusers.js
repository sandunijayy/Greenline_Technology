...
const Umusers = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const response = await axios.get(
        "http://localhost:3000/api/v1/auth/usermanager/users"
      );
      setUsers(response.data.users); // assuming the response directly contains users
      setLoading(false);
      setError(null);
    } catch (error) {
  ...
  const updateRole = async (userId, newRole) => {
    try {
      await axios.put(`/api/v1/auth/usermanager/users/role/${userId}`, {
        role: newRole,
      });
      fetchUsers();
  ...
  const handleDeleteUser = async (userId) => {
    ...
    if (confirmDelete) {
      try {
        await axios.delete(`/api/v1/auth/usermanager/users/delete/${userId}`);
        fetchUsers();
